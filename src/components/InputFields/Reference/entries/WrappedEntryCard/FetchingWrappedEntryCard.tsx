...
import { ContentType, Entry, FieldExtensionSDK, NavigatorSlideInfo, RenderDragFn } from '../../types'
...
export function FetchingWrappedEntryCard(props: EntryCardReferenceEditorProps) {
  ...
  const [contentType, setContentType] = React.useState<ContentType | undefined>(undefined)
  ...
  React.useEffect(() => {
    if (entry) {
      props.sdk.cma.contentType.get({ contentTypeId: entry.sys.contentType.sys.id })
        .then((contentType) => {
