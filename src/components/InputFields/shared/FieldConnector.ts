...
import { EntryFieldAPI, FieldAPI, ValidationError } from "@contentful/app-sdk";
...
> {
  ...
  componentDidMount() {
    ...
    // this.unsubscribeErrors = field.onSchemaErrorsChanged(
    //   (errors: ValidationError[]) => {
    //     this.setState({
    //       errors: errors || [],
    //     });
    //   }
    // );
    this.unsubscribeDisabled = field.onIsDisabledChanged(
      (disabled: boolean) => {
        this.setState({
          disabled,
        });
    ...
    this.unsubscribeValue = field.onValueChanged(
      (value: ValueType | Nullable) => {
        this.setState((currentState) => {
          const isLocalValueChange = this.props.isEqualValues(
            value,
            currentState.value
          );
